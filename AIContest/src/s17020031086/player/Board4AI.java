...
import static s17020031086.player.Road._FORWARD;
import java.util.ArrayList;
import core.board.Board;
import core.board.PieceColor;
import core.game.Move;
import s17020031086.player.Road;
...
public class Board4AI extends Board {
	...
	/** 走法生成时获取棋点的宽度 */
	public final static int POINTSWIDTH=30;
	...
	int countAllThreats(PieceColor color) {
		...
		int pos_start = roadList.get(0).getStartPos();
		...
		for (int i = 0; i < five.size(); i++){
			 pos_start = five.get(i).getStartPos();
			 dir = five.get(i).getDir();
			for (int j = 0; j < 6; j++){
		...
		for (int i = 0; i < four.size(); i++){
			 pos_start = four.get(i).getStartPos();
			 dir = four.get(i).getDir();
			for (int j = 0; j < 6; j++){
				int pos = pos_start + _FORWARD[dir] * j;
				if(get(pos) != EMPTY) continue;
