...
public class ConnectedGoFrame extends GoFrame
{
		...
		boolean kibitzwindow, boolean canteach)
	{
		...
		score
			.add(new MenuItemAction(this, Global.resourceString("Local_Count")));
		score.addSeparator();
		...
		Options = options;
		options.add(Silent = new CheckboxMenuItemAction(this, Global
		...
		{
			options.addSeparator();
			options.add(KibitzWindow = new CheckboxMenuItemAction(this, Global
				.resourceString("Kibitz_Window")));
		...
		if (kibitzwindow)
		{
			AllComments = new MyTextArea("", 0, 0,
		...
		if ( !TimerInTitle)
		{
			SimplePanel btl = new SimplePanel(TL = new OutputLabel(Global
				.resourceString("Start")), 80, new OutputLabel(""), 20);
			bpp.add(btl);
		}
		BP.add("South", bpp);
		// Text Area
		...
		JPanel bcp = new BoardCommentPanel(new Panel3D(BP), new Panel3D(cp), B);
		add("Center", bcp);
	...
	{
		if (s.equals("send"))
			doAction(Global.resourceString("Send"));
	...
	public void itemAction (String o, boolean flag)
	{
		if (Global.resourceString("Kibitz_Window").equals(o))
		{
			if (KibitzWindow.getState())
				CommentPanelLayout.show(CommentPanel, "AllComments");
			else CommentPanelLayout.show(CommentPanel, "Comment");
