...
//
	...
	// set move actions in the node
	{
		...
		if (number % 2 == 0) {	//退回的是最后一步的倒数第2子
			//重新绘制最后一步的上一步的两个子
			if (currentNode.parentPos() == null) return;
			// 重新绘制倒数第2个子, 也即这个走步的第1个子	
			Node n2 = currentNode.parentPos().node();
			drawNode(n2, true);
			
			// 重新绘制倒数第1个子，也即这个走步的第2个子
			Node n = currentNode.node();
			drawNode(n, false);
		}
		else { //退回的是最后一步的倒数第1子, currentNode是最后一步的倒数第1个子	
			// 重新绘制倒数第2个子
			Node n = currentNode.node();
			drawNode(n, true);
	...
	{
		T.top().setaction("PB", black, true);//持黑者的名字
		T.top().setaction("BR", blackrank, true);//执黑者的段位，水平
		T.top().setaction("PW", white, true);//执白者的名字
		T.top().setaction("WR", whiterank, true);//执白者的段位
		T.top().setaction("KM", komi, true);//黑棋贴子
		T.top().setaction("HA", handicap, true);//是否让子
		T.top().setaction("GN", white + "-" + black, true);//棋谱文件的名字
		T.top().setaction("DT", new Date().toString());//对弈时间
	}
