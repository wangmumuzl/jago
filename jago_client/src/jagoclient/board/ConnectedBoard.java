...
{	ConnectedGoFrame CGF;
	...
	{	if (currentNode.isMain() && CGF.wantsmove()) return;
...
//			GF.getParameter("preventko",true)) return;
		if (currentNode.isMain() && CGF.wantsmove())
		{	if (CGF.moveset(i,j))
			{	sendi=i; sendj=j;
				update(i,j); copy();
				MyColor=P.color();
	...
	*/
	...
	{	if (currentNode.haschildren()) return;
		...
		if (CGF.wantsmove() && ((Node)currentNode.content()).main())
		{	CGF.moveset(i0,j0);
		}
	...
	*/
	...
	{	if (currentNode.haschildren()) return;
		if (GF.blocked() && currentNode.isMain()) return;
		if (currentNode.isMain() && CGF.wantsmove())
		{	CGF.movepass(); return;
		}
		super.pass();
	...
	*/
	...
	{	if (currentNode.isLastMain() && CGF.wantsmove()) return;
		super.insertnode();
	}
